...
for id_mvt, mvt_name in enumerate(movement_to_analyse):
    ...
    for id_name, name in enumerate(temp_liste_name):
        ...
        for file in fichiers_mat_subject:
            # data = load_and_interpolate_for_point(file)
            (data,
             subject_expertise,
             laterality,
             length_segment,
        ...
        plt.subplot(4, 1, 1)
        plt.plot(time, subject_velocities[:, 0], label='dPelvis_X')
        plt.xlabel('Time')
        ...
        plt.subplot(4, 1, 2)
        plt.plot(time, subject_velocities[:, 1], label='dPelvis_Y')
        plt.xlabel('Time')
        plt.ylabel('Angular Velocity (degrees/s)')
        ...
        plt.subplot(4, 1, 3)
        plt.plot(time, subject_velocities[:, 2], label='dPelvis_Z')
        plt.xlabel('Time')
        plt.ylabel('Angular Velocity (degrees/s)')
        ...
        plt.subplot(4, 1, 4)
        plt.plot(time, subject_velocities[:, 3], label='Global Velocity', color='m')
        plt.xlabel('Time')
